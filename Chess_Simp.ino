...
bool legalOrNot;
int hintValue = 0;  // 0 = no hint requested, 1 = hint request, 2 = start new game, 3 = shutdown, 4 = board position check
int posreqCalled = 0;
...
void showBoardLayout(){
  // Serial.println("Showing board layout");
...
String getCoordinates(int buttonAlreadyPressed){
  ...
  while (column == "x"){
    ...
    if (hintValue == 4) {
      // Serial.println("Sending board position request to RPi...");
      sendToPi("", "b");
      showBoardLayout();
      showChessboardMarkings();
...
int detectButton(){
  ...
  while (true) {  
    if (digitalRead(5) == LOW){
      ...
      //Serial.println("Button connected to OK detected");
      val = 9;
      break;
...
// Counter delay to ride through switch debounces
 ...
 while (digitalRead(3) == LOW) {
  ...
  if (digitalRead(5) == LOW) { // if "A1" button is pressed while HINT is pressed
    hintValue = 4; // set value for board position check
    posreqCalled = 1;
    for (long i = 0; i <= 200000; i++) {
      int x = 0;
    }
