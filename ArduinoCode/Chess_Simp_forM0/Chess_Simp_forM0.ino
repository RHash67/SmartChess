...
//Mar 2024 - modified hint interrupt call to a function to provide better program flow control.
//Fixed issues with checkmate and pawn promotion detection/handling.
//Added routines to show the board layout, and to allow player to play as black.
//Modified new game routine to allow player to change level and computer move time.
...
void loop() {
  ...
  if (compFirstMove == true) {  // if player is black, then computer goes first
    cGoFirst();
    compFirstMove = false;
  }
  humansMove = humansGo();
  if (newGameStart == false) {
    // PROGRAM POINT A (corresponds with Point A in StartChessGame.py)
...
String humansGo(){
  ...
  while (moveConfirmed == false){ 
    ...
    controlPanelLED.fill(cpBLACK, 0, 22);  // turn off the control panel lights
    controlPanelLED.fill(cpWHITE, 0, 4); //turn on the control panels co-ordinate lights
    controlPanelLED.show();
...
bool checkPiForError(){  //check 30 times during the next 02 seconds to see if we received an error from maxchess on the pi - if so run errorFromPi()
...
String getCoordinates(int buttonAlreadyPressed) {
  ...
  if (newGameStart == true) {  // kick out of routine if new game requested
    return "";
  }
...
void setUpGame(){
 ...
 if (gameMode == "Stockfish"){
  ...
  String gameTimeout;
  int pColorButton = 0;  // 1 = player white, 2 = player black
  ...
  SerialUSB.print("Set player's color: ");
  chessboardLEDS.fill(BLACK, 0);
  chessboardLEDS.drawFastHLine(2,2,3,MAGENTA);
  chessboardLEDS.drawFastVLine(2,2,5,MAGENTA);
  chessboardLEDS.drawFastHLine(2,6,3,MAGENTA);
  chessboardLEDS.show();
  controlPanelLED.fill(cpBLACK, 0, 6);
  controlPanelLED.setPixelColor(0, 255, 255, 255);  // light up the 1 & 2 buttons
  controlPanelLED.show();
  while (pColorButton == 0) {
    pColorButton = detectButton();
    waitMsTime(buttonDebounceTime);
    if ((pColorButton == 0) || (pColorButton > 2)) {
      pColorButton = 0;
    } else break;
  }
  waitMsTime(500);
  ...
  SerialUSB.println(playerColor);
  sendToPi(playerColor, "-");
...
void waitMsTime(int msWait) { // changed delay to this function to avoid using a blocking type command
  unsigned long ctime = millis();
  while ((millis() - ctime) < msWait) {
    int x = 0;
  }
}
